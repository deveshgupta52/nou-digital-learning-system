...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    ...
    const res = axios.get(`http://localhost:3000/students/${email}`)
    .then((res) => {
      setStData(res.data);
      setLoading(false);
    })
  ...
  const studentData = {
    name: stData.name,
    id: stData.rollno,
    email: stData.emailaddress,
    phone: stData.contactno,
    dob: stData.dob.toString().slice(0, 10),
    gender: stData.gender,
    course: stData.courseCode,
    branch: stData.branch,
    year: stData.year,
    semester: "1", 
    enrollmentDate: stData.regdate.toString().slice(0, 10),
    attendance: 85,
    gpa: 1,
    permanentAddress: stData.address,
    currentAddress: stData.address
  };
  ...
  const renderContent = () => {
    ...
    switch (activeSection) {
      ...
      case 'exams':
        return <Examination handleStartExam={handleStartExam}/>
      case 'questionbank':
        return <QuestionBank 
      ...
      <div className="d-flex">
        ...
        <nav className="sidebar position-fixed top-0 start-0 h-100 p-3" style={{ width: '250px', zIndex: 1000 }}>
          ...
          <div className="position-absolute bottom-0 start-0 w-100 p-3">
            <button className="logout-btn btn text-white w-100 py-2" onClick={() => {
