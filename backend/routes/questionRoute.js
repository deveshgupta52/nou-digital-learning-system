...
questionRoute.post('', async (req, res) => {
  try {
    const { question, subject, difficulty, marks, options, correctAnswer } = req.body;
    // basic validation
    if (!question) return res.status(400).json({ message: "Question text required" });
    if (!Array.isArray(options) || options.length < 2) return res.status(400).json({ message: "Options must be an array (min 2)" });
    const q = new Question({ question, subject, difficulty, marks, options, correctAnswer });
